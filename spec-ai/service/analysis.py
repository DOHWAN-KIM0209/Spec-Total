...
from ai.resnet_proc import ResNet9  # Changed from resnet18_model to resnet9_model
...
def _facial_emotional_recognition(record: Analysis) -> None:
    ...
    # Extract frames
    frame_list = ResNet9.extract_frames(video_path, msec=(1000 // settings.FPS))  # Changed to resnet9_model
    ...
    ResNet9.save_thumbnail(frame_list[5], thumbnail_path)  # Changed to resnet9_model

    # Detect face from the frames
    face_list = []
    for img in frame_list:
        _, face_img = ResNet9.detect_faces(img, (224, 224))  # Changed to resnet9_model
    ...
    for img in face_list:
        pred = None
        if img is not None:
            pred = ResNet9.predict(Image.fromarray(img))  # Changed to resnet9_model
